...
import NetworkConstructorsAdaptive as networks
import os 
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = x.cpu()
        #self.matrixMean = self.matrixMean.cpu()
        #self.matrixStd = self.matrixStd.cpu()
        x = x.cuda()
        #self.matrixMean = self.matrixMean.cuda()
        #self.matrixStd = self.matrixStd.cuda()
        #print(x.get_device(), self.matrixMean.get_device(), self.matrixStd.get_device())  # Cuda = 0, CPU = -1
        ...
        out = self.softmax(self.sm(out))
        #out = F.softmax(self.sm(out), dim = out.size(0))
        return out
...
def train_resnet56():
    ...
    # Save weights attributes
    saveTag = "Trained_ResNet_56"
    ...
    cifar10_train_loader = torch.utils.data.DataLoader(datasets.CIFAR10(root='./data', train=True, transform=transformTrain, download=True), batch_size=batchSize, shuffle=True, pin_memory=True) 
    cifar10_val_loader = torch.utils.data.DataLoader(datasets.CIFAR10(root='./data', transform=transformTest, train=False), batch_size=batchSize, shuffle=False, pin_memory=True) 

    # Initialize carlini network 
    resnet_56 = resnet56(imgSize, num_classes).to(device)

    # Loss and optimizer
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(resnet_56.parameters(), lr = learning_rate)

    # Train and validate
    train(num_epochs, num_classes, resnet_56, cifar10_train_loader, device, optimizer, criterion)
    #get_train_acc()
    print("------------------------------------")
    print("After Training: ")  
    print("------------------------------------")
    train_acc = get_train_acc(cifar10_train_loader, resnet_56, device)
    val_acc = get_val_acc(cifar10_val_loader, resnet_56, device)
    #print("Training Accuracy: ", train_acc)
    #print("Validation Accuracy: ", val_acc)

    # Save trained model
    torch.save({'epoch': num_epochs, 'state_dict': resnet_56.state_dict(), 'bestAcc': val_acc}, os.path.join(saveDir, saveTag+'.th')) 
